...
from ucs.inject import get_file_upload_received_dao
...
async def test_dto_to_event(joint_fixture: JointFixture):
    ...
    The expected payload is a FileUploadReceived model dump.
    """
    config = joint_fixture.config
    async with get_file_upload_received_dao(config=config) as outbox_dao:
        ...
        async with (
            ...
            joint_fixture.kafka.expect_events(
                ...
                in_topic=config.file_upload_received_topic,
            ),
        ):
...
class DummySubTranslator(DaoSubscriberProtocol):
    """A class that consumes FileUploadReceived events."""
    ...
    async def changed(self, resource_id: str, update: FileUploadReceived) -> None:
...
async def test_partial_publish(joint_fixture: JointFixture):
    ...
    collection = db[joint_fixture.config.file_upload_received_collection]
    ...
    async with set_new_correlation_id():
        async with joint_fixture.kafka.expect_events(
            ...
            await joint_fixture.file_upload_received_dao.insert(published_event)
    ...
    async with joint_fixture.kafka.expect_events(
        ...
        await joint_fixture.file_upload_received_dao.publish_pending()
