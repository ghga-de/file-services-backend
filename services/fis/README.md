...
```bash
docker pull ghga/file-ingest-service:3.2.0
```

Or you can build the container yourself from the [`./Dockerfile`](./Dockerfile):
```bash
# Execute in the repo's root dir:
docker build -t ghga/file-ingest-service:3.2.0 .
```
...
docker run -p 8080:8080 ghga/file-ingest-service:3.2.0 --help
```
...
- **`source_bucket_id`**: ID of the bucket the object(s) corresponding to the upload metadata have been uploaded to. Only set this as a fallback option for metadata that does not contain the bucket ID. Default: `null`.
