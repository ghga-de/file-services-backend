...
```bash
docker pull ghga/encryption-key-store-service:2.0.1
```

Or you can build the container yourself from the [`./Dockerfile`](./Dockerfile):
```bash
# Execute in the repo's root dir:
docker build -t ghga/encryption-key-store-service:2.0.1 .
```
...
docker run -p 8080:8080 ghga/encryption-key-store-service:2.0.1 --help
```
