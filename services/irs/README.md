...
```bash
docker pull ghga/interrogation-room-service:6.1.0
```

Or you can build the container yourself from the [`./Dockerfile`](./Dockerfile):
```bash
# Execute in the repo's root dir:
docker build -t ghga/interrogation-room-service:6.1.0 .
```
...
docker run -p 8080:8080 ghga/interrogation-room-service:6.1.0 --help
```
