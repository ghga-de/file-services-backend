...
```bash
docker pull ghga/interrogation-room-service:7.0.2
```

Or you can build the container yourself from the [`./Dockerfile`](./Dockerfile):
```bash
# Execute in the repo's root dir:
docker build -t ghga/interrogation-room-service:7.0.2 .
```
...
docker run -p 8080:8080 ghga/interrogation-room-service:7.0.2 --help
```
