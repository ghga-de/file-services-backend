...
```bash
docker pull ghga/purge-controller-service:7.0.0
```

Or you can build the container yourself from the [`./Dockerfile`](./Dockerfile):
```bash
# Execute in the repo's root dir:
docker build -t ghga/purge-controller-service:7.0.0 .
```
...
docker run -p 8080:8080 ghga/purge-controller-service:7.0.0 --help
```
