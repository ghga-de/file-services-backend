...
```bash
docker pull ghga/internal-file-registry-service:3.0.3
```

Or you can build the container yourself from the [`./Dockerfile`](./Dockerfile):
```bash
# Execute in the repo's root dir:
docker build -t ghga/internal-file-registry-service:3.0.3 .
```
...
docker run -p 8080:8080 ghga/internal-file-registry-service:3.0.3 --help
```
