![tests](https://github.com/ghga-de/encryption-key-store-service/actions/workflows/unit_and_int_tests.yaml/badge.svg)
...
The Encryption Key Store Service (EKS) handles storage and retrieval of encryption/decryption secrets, extraction of information from and creation of personalized Crypt4GH file envelopes.
...
docker pull ghga/encryption-key-store-service:<version>
```
...
docker build -t ghga/encryption-key-store-service:<version> .
```
...
docker run -p 8080:8080 ghga/encryption-key-store-service:<version>
```
...
- specify the path to via the `ENCRYPTION_KEY_STORE_CONFIG_YAML` env variable
- rename it to `.encryption_key_store.yaml` and place it into one of the following locations:
  - the current working directory were you are execute the service (on unix: `./.encryption_key_store.yaml`)
  - your home directory (on unix: `~/.encryption_key_store.yaml`)
...
For naming the environment variables, just prefix the parameter name with `ENCRYPTION_KEY_STORE_`,
e.g. for the `host` set an environment variable named `ENCRYPTION_KEY_STORE_HOST`
